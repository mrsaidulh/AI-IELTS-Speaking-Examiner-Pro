      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <div className="md:col-span-2 bg-slate-950/60 rounded-xl p-4 border border-slate-800 space-y-3">
          ...
          <div className="space-y-2 pt-1">
            <span className="text-xs font-semibold text-slate-300">Standard Test Questions:</span>
            <ul className="space-y-1.5">
              {currentTopic.questions.map((q, idx) => (
                <li key={idx} className="flex items-start justify-between group text-xs text-slate-300 bg-slate-900/60 p-2 rounded-lg border border-slate-800 hover:border-indigo-500/50 transition-colors">
                  <span className="flex items-center space-x-2">
                    <span className="w-4 h-4 rounded-full bg-indigo-950 text-indigo-300 text-[10px] flex items-center justify-center font-mono border border-indigo-800">
                      {idx + 1}
                    </span>
                    <span>{q}</span>
                  </span>
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 text-[10px] text-indigo-400 hover:text-indigo-300 bg-slate-800 px-2 py-0.5 rounded border border-slate-700 transition-opacity"
                  >
                    Practice This
                  </button>
                </li>
